...
async function selectMember(memberId) {
    ...
    // Aktifkan Tampilan Chat di Mobile (Slide ke Chat View)
    ...
    // Ubah Tema Background Berdasarkan Team / Generasi Member
    ...
    // Fetch pesan real-time Telegram dari backend /pm/getpmmessages (Limit 150 pesan awal untuk fast load)
    const session = getCookie("user_session_pm");
    try {
        const res = await fetch(BACKEND_URL + "/pm/getpmmessages", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ session_cookie: session, folder_name: activeMember.name, limit: 150, offset_id: 0 })
        });
        const data = await res.json();
        ...
        if (res.ok && data.ok) {
            ...
            if (msgs.length > 0) {
                ...
                if (msgs.length < 150) hasMoreMessages = false;
            } else {
...
async function loadOlderMessages() {
    ...
    try {
        ...
        if (res.ok && data.ok && data.messages && data.messages.length > 0) {
            ...
            if (olderMsgs.length < 150) hasMoreMessages = false;
...
// --- CANVAS BINTANG OPTIMIZED (30 FPS & JEDA OTOMATIS SAAT SCROLL) ---
function setupStarCanvas() {
    ...
    // Deteksi scroll untuk menjeda animasi agar CPU & GPU HP tetap dingin
    const chatMessages = document.getElementById("chatMessages");
    if (chatMessages) {
    ...
    let lastTime = 0;
    const fps = 30; // Limit ke 30 FPS agar hemat daya & baterai
    ...
    function animate(currentTime) {
        ...
        if (isScrolling || delta < interval) return;
...
function setupWebSocket() {
    ...
    try {
        ...
        ws.onopen = function() {
            wsRetryCount = 0;
            // State Sync: Jika internet mati & terhubung kembali, sync otomatis pesan terbaru yang terlewat
            if (activeMember) {
