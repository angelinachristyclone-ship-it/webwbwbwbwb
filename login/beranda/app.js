        ...
        html += `
            <div class="member-item ${teamClass} ${activeMember && activeMember.id === mem.id ? 'active' : ''}" onclick="selectMember('${mem.id}')" role="option" aria-selected="${activeMember && activeMember.id === mem.id}">
                <div class="member-avatar-wrap">
                    ...
                    ${!isSubbed ? '<div class="lock-icon-badge">ðŸ”’</div>' : ''}
                </div>
                <div class="member-info">
                    <div class="member-name">${mem.name}</div>
                    <div class="member-status ${isSubbed ? 'active-sub' : ''}">
                        ${isSubbed ? 'âœ“ Berlangganan' : 'ðŸ”’ Belum Berlangganan'}
                    </div>
                </div>
            </div>
        `;
...
window.selectMember = async function(memberId) {
    ...
    document.getElementById("headerSub").innerText = isSubbed ? "Aktif Berlangganan" : "ðŸ”’ Belum Berlangganan";
    ...
    if (!isSubbed) {
        chatMessages.innerHTML = `
            <div class="unsubscribed-notice">
                <h3>ðŸ”’ Belum Berlangganan ${activeMember.name}</h3>
    ...
    try {
        ...
        if (res.ok && data.ok) {
            ...
            chatMessages.innerHTML = `<div class="unsubscribed-notice"><h3>âš ï¸ Notice</h3><p>${data.msg}</p></div>`;
        }
    } catch (e) {
        if (e.name !== "AbortError") {
            chatMessages.innerHTML = `<div style="margin:auto; color:#ff4d4d;">âŒ Gagal memuat pesan dari Telegram.</div>`;
        }
    }
}
    ...
    if (mediaType === 'video') {
        ...
        container.innerHTML = `
            <div style="background:rgba(20,25,35,0.95); padding:30px; border-radius:16px; width:320px; text-align:center; border:1px solid var(--border-color);">
                <div style="font-size:40px; margin-bottom:12px;">ðŸŽµ</div>
...
async function directDownload(btnElem, mediaUrl, filename) {
    ...
    requestAnimationFrame(async () => {
        try {
            ...
            if (btnElem) {
                btnElem.innerText = "âœ… Tersimpan!";
                setTimeout(() => {
        ...
        if (currentMediaTab === 'photo') {
            ...
            html += `
                <div style="position:relative; cursor:pointer;" onclick="openLightbox('${mediaSrc}', 'video', 'pm_video_${msg.id}.mp4')">
                    ...
                    <div style="position:absolute; inset:0; display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,0.4); border-radius:8px; color:#fff; font-size:16px;">â–¶</div>
                </div>`;
            ...
            html += `
                <div style="grid-column: span 3; background:rgba(255,255,255,0.06); padding:10px 14px; border-radius:8px; display:flex; justify-content:space-between; align-items:center; cursor:pointer; border:1px solid var(--border-color);" onclick="openLightbox('${mediaSrc}', 'audio', 'pm_audio_${msg.id}.m4a')">
                    <span style="font-size:12px; color:#fff;">ðŸŽµ Voice Note [${msg.timestamp || 'Audio'}]</span>
                    <span style="font-size:11px; color:var(--accent-blue); font-weight:600;">â–¶ Putar</span>
                </div>`;
        }
    });
        ...
        if (msg.media_type === 'photo') {
            if (isRecent) {
                mediaTag = `
                    <div style="position:relative; min-height:140px;">
                        ...
                        <button class="save-media-btn" onclick="directDownload(this, '${mediaSrc}', 'pm_photo_${msg.id}.jpg')">ðŸ’¾ Simpan Foto</button>
                    </div>`;
            } else {
                mediaTag = `
                    <div style="position:relative; min-height:140px;">
            ...
            if (isRecent) {
                mediaTag = `
                    <div style="position:relative; min-height:140px;">
                        ...
                        <button class="save-media-btn" onclick="directDownload(this, '${mediaSrc}', 'pm_video_${msg.id}.mp4')">ðŸ’¾ Simpan Video</button>
                    </div>`;
            } else {
                mediaTag = `
                    <div style="position:relative; min-height:140px;">
            ...
            mediaTag = `
                <div style="background:rgba(0,0,0,0.2); padding:8px; border-radius:8px; margin-bottom:6px;">
                    ...
                    <button class="save-media-btn" style="font-size:11px;" onclick="directDownload(this, '${mediaSrc}', 'pm_audio_${msg.id}.m4a')">ðŸ’¾ Simpan Voice/Audio</button>
                </div>`;
        }
    }
...
async function loadOlderMessages() {
    ...
    loader.innerText = "â³ Memuat pesan lama...";
...
function triggerWebNotification(memberName, textContent) {
    if ("Notification" in window && Notification.permission === "granted") {
        const title = `ðŸ“© Pesan PM Baru dari ${memberName}`;
        const options = {
